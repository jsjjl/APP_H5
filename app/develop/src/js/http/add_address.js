...
$(function(){
	...
	function bindEvents() {
		...
		function enableSave() {
			...
			_len_cityName=$('.cityName').html();
			_len_areaName=$('.areaName').html();
			if (_len_username&&_len_tel&&_len_address&&_len_provinceName&&_len_cityName&&_len_areaName) {
...
address_param["detailAddress"] = $(".txt_address").val();
	...
	var url = URLS.SERVER_URL + URLS.userAddressAdd;
	var data = address_param;
	var paramsStr = '';
	for (v in address_param) {
		paramsStr = paramsStr + v + '=' + address_param[v] + '&';
	}
	console.log(URLS.SERVER_URL + URLS.userAddressAdd + '?' + paramsStr.substr(0, paramsStr.length - 1));
	//成功回调函数
	var success = function(data){
